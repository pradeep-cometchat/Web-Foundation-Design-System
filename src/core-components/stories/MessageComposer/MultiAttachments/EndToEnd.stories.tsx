...
} from "../../../../base-components/components/ActionSheet";
import {
  UsageDoc,
  DropOverlay,
  CIconFormatting,
...
const sendBase: React.CSSProperties = {
  ...
  justifyContent: "center",
  boxShadow:
    "0px 1px 2px color-mix(in srgb, var(--cometchat-static-black) 5%, transparent)",
  flexShrink: 0,
};
...
const DOC_EXT: Record<string, DocKind> = {
  pdf: "pdf",
  doc: "doc",
  docx: "doc",
  xls: "xls",
...
const SEED: Msg[] = [
  ...
  },
  {
    ...
    variant: "sent",
    groups: [
      { t: "audio", files: [{ name: "Voice-reply.mp3", meta: "00:18" }] },
    ],
    time: "4:53 pm",
    status: "read",
  },
  {
    id: 4,
    variant: "received",
    groups: [{ t: "media", count: 1, total: 1, video: false }],
...
function EndToEndChat({
  ...
  function classify(file: File): Pending {
    ...
    if (file.type.startsWith("audio/"))
      return { id, kind: "audio", name: file.name, meta: "00:30" };
    ...
    return {
      ...
      docType,
      name: file.name,
      meta: `${docType.toUpperCase()} · ${formatSize(file.size)}`,
    };
  }
  ...
  const queue = (items: Pending[]) => {
    ...
    loading.forEach((it) =>
      window.setTimeout(
        () =>
          setPending((p) =>
            p.map((x) => (x.id === it.id ? { ...x, loading: false } : x)),
          ),
        1200,
      ),
    );
  ...
  const addFiles = (files: FileList | File[]) =>
    queue(Array.from(files).map(classify));
    ...
    const item: Pending =
      kind === "image"
        ? {
            ...
            name: "Photo.jpg",
            meta: "",
            src: SAMPLE_IMAGES[id % SAMPLE_IMAGES.length],
          }
        : kind === "video"
  ...
  const sheetItems = [
    ...
    },
    {
      icon: <VideocamIcon />,
      label: "Attach Video",
      onClick: () => addSample("video"),
    },
    {
      icon: <PlayCircleIcon />,
      label: "Attach Audio",
      onClick: () => addSample("audio"),
    },
    {
  ...
  function buildGroups(items: Pending[]): Group[] {
    ...
    if (auds.length)
      g.push({
        t: "audio",
        files: auds.map((a) => ({ name: a.name, meta: a.meta || "00:30" })),
      });
    return g;
  }
  ...
  function send() {
    ...
    window.setTimeout(() => {
      setMessages((ms) =>
        ms.map((m) =>
          m.id === id ? { ...m, state: "default", status: "read" } : m,
        ),
      );
    }, 1700);
  }
      ...
      m.groups.length === 0
        ...
        : m.groups.map((g, i) => {
            ...
            const common = {
              ...
              time: m.time,
              status: m.status,
              state: (m.state === "uploading" ? "uploading" : "default") as
                "uploading" | "default",
            ...
            if (g.t === "media")
              return (
                <MultiAttachmentBubble
                  ...
                  totalImages={g.total}
                  videoAt={
                    g.video ? Array.from({ length: g.count }, (_, k) => k) : []
                  }
                />
              );
            if (g.t === "files")
              return (
                <MultiAttachmentBubble key={i} {...common} files={g.files} />
              );
            return (
              <MultiAttachmentBubble
                key={i}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {replyTo && (
          <div
            style={{
              margin:
                "var(--cometchat-spacing-1) 0 var(--cometchat-spacing-1-5)",
            }}
          >
            <MessagePreview
              mode="reply"
              senderName={quoteFrom(replyTo).name}
              messageText={quoteSummary(quoteFrom(replyTo)) || "Message"}
      ...
      {/* Panel-scoped drag overlay */}
      {dragging && !onComposer && <DropOverlay chatName={CHAT_NAME} />}
    </div>
  );
}
 ...
 *  footer only — the message thread stays visible. */
export const ChatComposerDrop: Story = {
