...
const IconSpinner = ({ size = 12 }: { size?: number }) => (
  <svg
    ...
    viewBox="0 0 12 12"
    fill="none"
    style={{ animation: "ma-spin 0.9s linear infinite" }}
  >
    <circle
      cx="6"
      cy="6"
...
function cornerFor(state: CardState, platform: Platform): CornerKind {
  ...
  if (state === "error" || state === "retry")
    return platform === "mobile" ? "remove" : state;
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
      <circle
        ...
        cy={size / 2}
        r={r}
        stroke="color-mix(in srgb, var(--cometchat-static-white) 30%, transparent)"
        strokeWidth={stroke}
        fill="none"
      />
      <circle
      ...
      {(loading || error || retry) && (
        <div
          style={{
            ...
            justifyContent: "center",
            background:
              "color-mix(in srgb, var(--cometchat-static-black) 62%, transparent)",
            color: "var(--cometchat-static-white)",
          }}
        >
          {loading ? (
            ...
            >
              {retry ? (
                <IconRetry size={Math.round(size * 0.24)} />
              ) : (
                <IconError size={Math.round(size * 0.24)} />
              )}
            </div>
          )}
        </div>
...
function AudioButton({
  ...
  return (
    ...
    >
      ...
      >
        {playing ? (
          <IconPause size={Math.round(size * 0.3)} />
        ) : (
    ...
    >
      <div
        style={{
          ...
          borderRadius: 14,
          overflow: "hidden",
          border: `1px solid ${failed ? "var(--cometchat-error-color)" : "var(--cometchat-border-color-default)"}`,
        }}
      >
        <img
          src={src}
          ...
          style={{
            ...
            height: "100%",
            objectFit: "cover",
            filter: dim ? "blur(2px) brightness(0.7)" : undefined,
          }}
        />
        {kind === "video" && !dim && (
          <>
    ...
    >
      <div
        style={{
          ...
          padding:
            "var(--cometchat-spacing) var(--cometchat-spacing-3) var(--cometchat-spacing-2-5)",
        }}
      >
        <span
          style={{
      ...
      <div
        style={{
          ...
          padding:
            "var(--cometchat-spacing-1) var(--cometchat-spacing-3) var(--cometchat-spacing-3)",
        }}
      >
        {children}
      </div>
...
function TypePage({
  ...
  render: (state: CardState, platform: Platform) => React.ReactNode;
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      <Section title="Mobile">
        <MobileFrame>
          {(["default", "loading", "error", "retry"] as CardState[]).map(
            (s) => (
              <div
                key={s}
...
export const Image: Story = {
  ...
  render: () => (
    <TypePage
      render={(s, p) => (
        <MediaTile kind="image" state={s} platform={p} src={SAMPLE_IMAGES[0]} />
      )}
    />
  ),
};
...
export const Video: Story = {
  ...
  render: () => (
    <TypePage
      render={(s, p) => (
        <MediaTile kind="video" state={s} platform={p} src={SAMPLE_IMAGES[1]} />
      )}
    />
  ),
};
...
export const Audio: Story = {
  ...
  render: () => (
    <>
      <TypePage
        render={(s, p) => (
          <AudioCard state={s} platform={p} name="Watch by Billie.mp3" />
        )}
      />
      <div
        style={{
